...
df = pd.read_csv("course-1/s4/BTCUSD_Candlestick_1_D_ASK_08.05.2017-16.10.2021.csv")
...
fig = go.Figure(data = [go.Candlestick(x=dpfl.index, open=dpfl["Open"], high = dpfl["High"], low=dpfl["Low"], close = dpfl["Close"]), 
                        ...
                        go.Scatter(x=dpfl.index, y=dpfl.ADX, line= dict(color='green', width=2), name="ADX", yaxis='y2')])
fig.show()
